...
void SSPIntIni(unsigned int isrAddr, unsigned char sspNum)
{
	if(sspNum == 0)
	{
		...
		VICIntEnable |= 1 << 10; //0x00000040;  // Enable ssp0 Interrupt
	}
	else 	if (sspNum == 1)
	{
		VICVectAddr11 = isrAddr;
		VICVectCntl11 = 0x20 | 11;     // use it for ssp1 Interrupt
		VICIntEnable |= 1 << 11; //0x00000040;  // Enable ssp1 Interrupt
	}
	*SSP_IMSC[ sspNum ]  = 0;
}

void SSPrxIntEnable(unsigned char sspNum)
{
	*SSP_IMSC[ sspNum ]  |= 1 << 2;
}
...
void SSPTimOrxDisable(unsigned char sspNum)
{
	*SSP_IMSC[ sspNum ]  &= ~2;
}
